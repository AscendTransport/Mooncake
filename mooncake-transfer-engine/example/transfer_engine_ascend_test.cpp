...
int initiator() {
    ...
    std::string FLAGS_local_server_name_new = hostname_port.first + ":" + std::to_string(new_port) + ":npu" + std::to_string(procRank);
    ...
    // size_t pos = FLAGS_sigment_id.find(':');

    // // get hostname
    // std::string hostname = FLAGS_segment_id.substr(0, pos);

    // // get old port
    // std::string old_port_str = FLAGS_segment_id.substr(pos + 1);
    // int old_port = std::stoi(old_port_str);
    // int used_port = old_port + procRank + 8;

    // // generate new string
    // std::string FLAGS_segment_id_new = hostname + ":" + std::to_string(used_port) + std::to_string(procRank);
