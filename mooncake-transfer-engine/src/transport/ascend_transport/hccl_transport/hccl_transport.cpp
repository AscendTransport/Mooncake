...
int HcclTransport::nonAggTransport(std::vector<Slice *> &slice_list,
    ...
    if (std::string(local_rank_info_.hostIp) + std::to_string(local_rank_info_.devicePhyId) ==
        std::string(remote_rank_info_.hostIp) + std::to_string(remote_rank_info_.devicePhyId)) {
        //D2H
        for (auto slice : slice_list) {
