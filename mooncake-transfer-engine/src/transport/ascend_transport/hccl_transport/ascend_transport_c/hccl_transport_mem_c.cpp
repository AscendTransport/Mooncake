...
int transportMemTask(RankInfo *local_rank_info, RankInfo *remote_rank_info,
    ...
    if (iter == target_key_to_connection_map_.end()) {
        ...
        // ret = aclrtGetDeviceInfo(&deviceInfo,
        // local_rank_info->deviceLogicId); if (ret) {
        //     LOG(ERROR) << "aclrtGetDeviceInfo failed, ret: " << ret;
        //     return ret;
        // }

        // LOG(INFO) << "NPU" << local_rank_info->deviceLogicId
...
int transportMemAccept(RankInfo *local_rank_info) {
    ...
    LOG(INFO) << "Received remote_control_info, deviceLogicId: "
              ...
              << ", hostIp: " << remote_control_info.hostIp
              << ", deviceIp: " << remote_control_info.deviceIp
    ...
    // ret = aclrtGetDeviceInfo(&deviceInfo, local_rank_info->deviceLogicId);
    // if (ret) {
    //     LOG(ERROR) << "aclrtGetDeviceInfo failed, ret: " << ret;
    //     return ret;
    // }
    ...
    if (!is_cross_hccs) {
        ...
        ret =
            acceptSocket(hccl_ctrl_socket, local_rank_info, remote_control_info,
                         baseTag_ + "ctrl", remoteDevIp, is_cross_hccs);
        if (ret) {
        ...
        ret =
            acceptSocket(hccl_data_socket, local_rank_info, remote_control_info,
                         baseTag_ + "data", remoteDevIp, is_cross_hccs);
        if (ret) {
    ...
    for (size_t i = 0; i < m_num; ++i) {
        ...
        if (!is_cross_hccs) {
            HcclMemGrantInfo grant_info;
            grant_info.remotePid = (int32_t)remote_control_info.devPid;
            grant_info.remoteSdid = remote_control_info.sdid;
            ret = HcclMemGrant(&buf, &grant_info);
            if (ret) {
                LOG(ERROR) << "HcclMemGrant failed, ret: " << ret
                           << ", addr: " << g_localMergeMem[i].addr
